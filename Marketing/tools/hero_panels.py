...
place(c, shot("panel-expanded.png"), W * 0.665, 360, 1560, radius_big=70, alpha_big=64)
place(c, shot("panel-compact.png"), W * 0.205, 480, 1120, radius_big=60, alpha_big=70)
place(c, shot("panel-mini.png"), W * 0.235, 870, 800, radius_big=55, alpha_big=66)
capy = shot("capy-glyph.png", "shots"); capy = capy.resize((150, int(150 * capy.height / capy.width)), Image.LANCZOS)
c.alpha_composite(capy, (int(W * 0.665 - capy.width / 2), 1120))
# Bottom right: the name, then the one line the email ends on.
dr = ImageDraw.Draw(c)
nf = font(72, "Bold"); ff = font(46, "Regular")
right = W - 150
out = os.path.join(SP, "..", "email", "hero-panels.jpg")
c.crop((0, 200, W, 1330)).convert("RGB").resize((2000, int(1130 * 2000 / W)), Image.LANCZOS).save(out, quality=86, optimize=True, progressive=True)
