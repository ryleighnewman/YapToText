...
struct WelcomeView: View {
    ...
    /// The master choice, presented right as modes are introduced: keep the analysis stage on
    /// (modes, formatting, cleanup) or turn it off for the fastest raw transcription. Default ON.
    /// Turning it off here dims the mode demo and Auto row below, so the effect is visible.
    private var analysisChoiceRow: some View {
        HStack(spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 1) {
                Text("Post-transcription analysis").font(.callout.weight(.medium))
                Text("Runs the mode system after each dictation - formatting, cleanup, and Auto. Leave it on to use modes; turn it off any time for the fastest raw transcription.")
                    .font(.caption).foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
            }
            Spacer(minLength: 8)
            Toggle("", isOn: Binding(get: { state.settings.aiCleanupEnabled },
                                     set: { state.settings.aiCleanupEnabled = $0 }))
    ...
    private func permissionCard(icon: String, title: String, required: Bool, granted: Bool,
        ...
        HStack(spacing: 14) {
            ...
            if granted {
                ...
                Button("Continue") { grant() }.buttonStyle(.solidSecondary).controlSize(.small)
            }
        }
        .padding(14)
        .yapGlass(in: RoundedRectangle(cornerRadius: Metrics.panelRadius, style: .continuous))
